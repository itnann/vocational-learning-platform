package com.yaojiuye.orders.api;
...
import com.yaojiuye.orders.util.SecurityUtil;
 ...
 * @Description 订单相关接口 生成订单, 生成二维码返回
 * @Date 2025/4/20 13:06
 * @Version V1.0
 */
...
public class OrderController {
    ...
    public void requestpay(String payNo, HttpServletResponse httpResponse) throws IOException {
        ...
        if ("601002".equals(status)) {
            GlobalException.cast("订单已支付，请勿重复支付。");
        }
        AlipayClient client = new DefaultAlipayClient(AlipayConfig.URL, APP_ID, APP_PRIVATE_KEY, AlipayConfig.FORMAT, AlipayConfig.CHARSET, ALIPAY_PUBLIC_KEY, AlipayConfig.SIGNTYPE);//获得初始化
        AlipayTradeWapPayRequest alipayRequest = new AlipayTradeWapPayRequest();//创建API对应的request
        //alipayRequest.setReturnUrl("http://domain.com/CallBack/return_url.jsp");
        alipayRequest.setNotifyUrl("http://yaojiuye.natapp1.cc/orders/receivenotify");//在公共参数中设置回跳和通知地址
    ...
    public void receivenotify(HttpServletRequest request, HttpServletResponse out) throws Exception {
    ...
    @ResponseBody
    public PageResult<XcOrders> queryOrders(PageParams pageParams) {
        SecurityUtil.XcUser user = SecurityUtil.getUser();
        String userId = user.getId();
        PageResult<XcOrders> payRecords = orderService.queryOrders(userId, pageParams);
        return payRecords;
    }
